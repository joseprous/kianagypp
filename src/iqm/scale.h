static void FUNCNAME(halvetexture)(uchar *src, uint sw, uint sh, uint stride, uchar *dst)
{
    for(uchar *yend = &src[sh*stride]; src < yend;)
    {
        ...
        {
#define OP(c, n) dst[n] = (uchar)((uint(src[n]) + uint(src[n+BPP]) + uint(src[stride+n]) + uint(src[stride+n+BPP]))>>2)
            PIXELOP
            #undef OP
        }
        src += stride;
...
static void FUNCNAME(shifttexture)(uchar *src, uint sw, uint sh, uint stride, uchar *dst, uint dw, uint dh)
{
    ...
    for(uchar *yend = &src[sh*stride]; src < yend;)
    {
        ...
        {        
            ...
            DEFPIXEL
            #undef OP
            for(uchar *ycur = src, *xend2 = &ycur[wfrac*BPP], *yend2 = &src[hfrac*stride]; 
                ycur < yend2; 
                ycur += stride, xend2 += stride)
            {
                for(uchar *xcur = ycur; xcur < xend2; xcur += BPP)
                {
            ...
            #define OP(c, n) dst[n] = (uchar)((c##t)>>tshift)
            PIXELOP
            #undef OP
        }
        src += (hfrac-1)*stride; 
...
static void FUNCNAME(scaletexture)(uchar *src, uint sw, uint sh, uint stride, uchar *dst, uint dw, uint dh)
{
    ...
    uint cscale = (uint)clamp(under, over - 12, 12),
        ascale = (uint)clamp(12 + under - over, 0, 24),
    ...
    for(uint y = 0; y < dh; y += hfrac)
    {
        ...
        for(uint x = 0; x < dw; x += wfrac, dst += BPP)
        {
            ...
            #define OP(c, n) dst[n] = (uchar)((c##t * area)>>dscale)
            PIXELOP
            #undef OP
        }
    }
